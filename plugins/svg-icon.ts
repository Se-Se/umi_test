...
export default (api: IApi) => {
    ...
    const svgoConfigDirFilePath = api.utils.winPath(join(srcDir!, '../svgo-config.json'));
    try {
        svgoConfigDirFile = require.resolve(svgoConfigDirFilePath);
    } catch (e) {
        console.log(api.utils.chalk.yellow(`[svg-icon]svgo-config.json doesn't find at ${svgoConfigDirFilePath}, svg-icon auto use default config`));
    }
